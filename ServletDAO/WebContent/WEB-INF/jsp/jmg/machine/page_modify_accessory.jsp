...
<html lang="${lang}">
	<head>
       <title>Modify Machine Accessory Page</title>
		...
		<div class="container-fluid">
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active" aria-current="page"><fmt:message key="label.updateAccessory"/></li>
				</ol>
			</nav>
			<h2 class="font-weight-light"><fmt:message key="label.machine"/>: <c:out value="${requestScope.machine.name.get(lang.toString())}"/>, <fmt:message key="label.accessory"/>: <c:out value="${requestScope.accessory.name.get(lang.toString())}"/></h2>
			...
			<div class="container-fluid px-4">
				<div class="row mx-lg-n5 justify-content-start">
					<div class="col py-3 px-lg-5">
						<form action="<c:url value="/web/machine/ModifyAccessoryData"/>" method="post">
							<input name="machine" type="hidden" value="${requestScope.machine.id}">
							<input name="accessory" type="hidden" value="${requestScope.accessory.id}">
							<div class="row">
								<div class="col">
									<label class="font-weight-bold" for="inputWeight"><fmt:message key="label.weight"/></label>
									<input id="inputWeight" type="number" step="any" value="${requestScope.accessory.weight}" name="weight" class="form-control" placeholder="kg">
								</div>
							</div>
							<br>
							<div class="row">
								...
								<input id="inputLength" type="number" step="any" value="${requestScope.accessory.length}"
									   name="length" class="form-control" placeholder="mt">
								</div>
							</div>
							<br>
							<div class="row">
								...
								<input id="inputDistance" type="number" step="any" value="${requestScope.accessory.distance}"
									   name="distance" class="form-control" placeholder="mt">
								</div>
							</div>
							<br>
							<div class="row">
								<div class="col">
									<p class="font-weight-bold"><fmt:message key="label.predefined"/></p>
									<div id="setPredefined" class="custom-control custom-radio custom-control-inline">
										<input type="radio" id="customRadioInline1" name="predefined" value="true" class="custom-control-input"
											   <c:if test="${requestScope.ballast.predefined}">checked</c:if>>
										<label class="custom-control-label" for="customRadioInline1"><fmt:message key="label.true"/></label>
									</div>
									<div class="custom-control custom-radio custom-control-inline">
										<input type="radio" id="customRadioInline2" name="predefined" value="false" class="custom-control-input"
											   <c:if test="${!requestScope.ballast.predefined}">checked</c:if>>
							...
							<br>
							<c:if test="${requestScope.accessory.headPositions.size() gt 0}">
								<div class="row justify-content-center">
									<div class="col">
										...
										<div id="inputHeadOffset" class="form-row">
											...
											<c:forEach items="${requestScope.accessory.headPositions}" var="headPosition">
												<div class="form-group col-md-6">
													<input type="number" name="position${i}" class="form-control" value="${headPosition.key}" placeholder="Position">
												</div>
												<div class="form-group col-md-6">
													<input type="number" name="offset${i}" class="form-control" value="${headPosition.value}" placeholder="Offset">
												</div>
												<c:set var="i" value="${i+1}"/>
											</c:forEach>
											<c:forEach var="i" begin="${i}" end="4">
												<div class="form-group col-md-6">
													<input type="number" name="newPosition${i}" class="form-control" value="" placeholder="Position">
												</div>
												<div class="form-group col-md-6">
													<input type="number" name="newOffset${i}" class="form-control" value="" placeholder="Offset">
												</div>
											</c:forEach>
										</div>
									</div>
					...
					<div class="col py-3 px-lg-5">
						<form action="<c:url value="/web/machine/ModifyAccessoryImage"/>" method="post" enctype="multipart/form-data">
							<input name="machine" type="hidden" value="${requestScope.machine.id}">
							<input name="accessory" type="hidden" value="${requestScope.accessory.id}">
							<c:choose>
								<c:when test="${empty requestScope.accessory.image}">
									<img class="rounded mx-auto d-block" src="<c:url value="/media/Image.png"/>" alt="..." width="200" height="200">
								</c:when>
								<c:otherwise>
									<img  class="rounded mx-auto d-block" src="data:image/png;base64,${requestScope.accessory.image}" alt="..." width="200" height="200">
								</c:otherwise>
							</c:choose>
